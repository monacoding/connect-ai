#!/usr/bin/env python3
"""Trend Sniper — pulls top YouTube videos for target keywords, asks the
OpenAI API to extract the algorithmic patterns, and writes a
planning report next to this script.

Shared keys (API key, OPENAI_BASE_URL, OPENAI_API_KEY, MODEL) come from youtube_account.json so
...
def main():
    ...
    if not target_keywords:
        ...
        sys.exit(1)
    openai_base = (_shared(cfg, acct, "OPENAI_BASE_URL", "https://api.openai.com/v1") or "https://api.openai.com/v1").rstrip("/")
    openai_key = (_shared(cfg, acct, "OPENAI_API_KEY", os.environ.get("OPENAI_API_KEY", "")) or "").strip()
    model = _shared(cfg, acct, "MODEL", "gpt-5.1") or "gpt-5.1"
...
"""
    ...
    if not openai_key:
        print("❌ OPENAI_API_KEY가 비어있어요. youtube_account.json 또는 환경변수에 입력하세요.")
        sys.exit(1)
