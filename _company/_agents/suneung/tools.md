...
_수능연구소 에이전트가 어떤 도구를 어디까지 자율적으로 쓸 수 있는지 정의합니다._
...
### `source_ingest`
공식/허가된 수능·모의평가 PDF/텍스트 자료 수집 + manifest 생성

- `enabled`: true
- `requires_credentials`: `config.md` 참조

### `md_convert`
PDF/텍스트를 Markdown과 AI 학습용 JSONL로 변환

- `enabled`: true
- `requires_credentials`: `config.md` 참조

### `curriculum_builder`
OpenAI API로 문항을 개념·성취기준·커리큘럼 모듈에 매핑

- `enabled`: true
- `requires_credentials`: `config.md` 참조

### `video_planner`
커리큘럼 모듈을 YouTube 강의 스크립트·샷리스트·메타데이터로 변환

- `enabled`: true
- `requires_credentials`: `config.md` 참조

### `pipeline_runner`
수집→변환→커리큘럼→영상기획 전체 파이프라인 실행
...
- 모든 외부 행동은 `_agents/suneung/activity.log`에 한 줄 기록 (감사용).
