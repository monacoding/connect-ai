...
_원영 에이전트가 어떤 도구를 어디까지 자율적으로 쓸 수 있는지 정의합니다._
...
### `web_search`
DuckDuckGo 웹 검색 — 제목·URL·스니펫 수집 (API 키 불필요)

- `enabled`: true
- `requires_credentials`: `config.md` 참조

### `page_fetcher`
URL 페이지 본문 텍스트 추출 + 출처 URL 기록
...
- 모든 외부 행동은 `_agents/crawler/activity.log`에 한 줄 기록 (감사용).
